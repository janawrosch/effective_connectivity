...
function [results]= auswGTE16(data)

%% Taking the time
dispstat('', 'init')
dispstat('GTE calculation started', 'timestamp','keepthis');
    ...
    for p = 1 :num_rows-z
        ...
        for s = 1:num_col-k;
            ...
            for o=k-1:-1:0
                ...
                words_k_y1 = words_k_y1 + words_array(z+p,s+o+1)*l;
                l=l*10;
            end
            
            xy1x1 = words_k_x*10^(k+1) ...
                +words_k_y1*10^(1)+words_k_x1;
            xy1 = words_k_x*10^k+ words_k_y1;
            x1x = words_k_x1*10^k+ words_k_x;
            
            l=1;
            for o=k-1:-1:0
                wordsr_k_x = wordsr_k_x + words_array(z+p,s+o)*l;
                wordsr_k_y1 = wordsr_k_y1 + words_array(z,s+o+1)*l;
                l=l*10;
            end
            
            %reverse
            rxy1x1 = wordsr_k_x*10^(k+1)...
                +wordsr_k_y1*10^(1)+wordsr_k_x1;
            rxy1 = wordsr_k_x*10^k+ wordsr_k_y1;
            rx1x = wordsr_k_x1*10^k+ wordsr_k_x;
            
            
            %% Generating counting list for one pair of cells ( Forwards ) %%%%%
            [counting_list]=generating_countinglist( counting_list,xy1x1,xy1,x1x,words_k_x);
            
            %% Generating Couintinng_list_r for one pari of cells ( Backwards) %%%
            [counting_list_r]=generating_countinglist(counting_list_r,rxy1x1,rxy1,rx1x,wordsr_k_x);
...
dispstat('GTE calculation finished', 'timestamp', 'keepthis')
% close(h);
